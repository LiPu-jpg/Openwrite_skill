...
└── data/novels/{novel_id}/     # 每部作品的运行时数据
    ...
    │   └── hierarchy.yaml     # 解析后的结构（含摘要、情感弧线、伏笔引用）
    ├── characters/             # 角色
    │   ├── cards/*.yaml       # 角色卡
    │   ├── profiles/*.md      # 角色详情
    │   └── relationships.md    # 角色关系矩阵（融合原 story/character_matrix）
    ├── world/                  # 世界观
    │   ├── rules.md            # 世界底层规则
    ...
    ├── foreshadowing/          # 伏笔图
    │   └── dag.yaml           # 伏笔 DAG（包含 pending_hooks）
    ├── style/                  # 风格合成结果
    │   ├── composed.md         # 最终合成风格
...
- `foreshadowing/dag.yaml` - 伏笔列表和状态

**已删除/融合：**
- `story/` 目录已删除，功能融合到对应模块
